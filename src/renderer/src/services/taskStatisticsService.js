...
export class TaskStatisticsService {
  ...
  static calculateCategoryCounts(tasks = []) {
    return {
      inbox: tasks.filter(t => t.status === 'todo' && !t.reminderTime).length,
      today: tasks.filter(t => (isToday(t.reminderTime) && t.status !== 'done') || t.status === 'doing').length,
      doing: tasks.filter(t => t.status === 'doing').length,
      paused: tasks.filter(t => t.status === 'paused').length,
      planned: tasks.filter(t => !!t.reminderTime).length,
      all: tasks.length,
