<template>
  <div>
    ...
    <!-- 状态变化统计 -->
    <div v-if="statusChangeStats" class="setting-stats-container">
      <h3 class="setting-stats-title">状态变化统计</h3>
      <div class="setting-stats-item">
        <span class="setting-stats-label">总状态变化次数:</span>
        <span class="setting-stats-value">{{ statusChangeStats.totalChanges }}</span>
      </div>
      <div class="setting-stats-item">
        <span class="setting-stats-label">今日状态变化:</span>
        <span class="setting-stats-value">{{ statusChangeStats.todayChanges }}</span>
      </div>
      <div class="setting-stats-item">
        <span class="setting-stats-label">本周状态变化:</span>
        <span class="setting-stats-value">{{ statusChangeStats.weekChanges }}</span>
      </div>
      <div v-if="statusChangeStats.transitions" class="setting-stats-transitions">
        <h4 class="setting-stats-subtitle">常见状态转换</h4>
        <div v-for="transition in statusChangeStats.transitions.slice(0, 5)" :key="transition.transition" class="setting-stats-item">
          <span class="setting-stats-label">{{ formatTransition(transition.transition) }}:</span>
          <span class="setting-stats-value">{{ transition.count }} 次</span>
        </div>
      </div>
    </div>

    <!-- 完成统计 -->
    <div v-if="completionStats" class="setting-stats-container">
      <h3 class="setting-stats-title">完成统计</h3>
      <div class="setting-stats-item">
        <span class="setting-stats-label">今日完成任务:</span>
        <span class="setting-stats-value">{{ completionStats.todayCompleted }}</span>
      </div>
      <div class="setting-stats-item">
        <span class="setting-stats-label">本周完成任务:</span>
        <span class="setting-stats-value">{{ completionStats.weekCompleted }}</span>
      </div>
      <div class="setting-stats-item">
        <span class="setting-stats-label">本月完成任务:</span>
        <span class="setting-stats-value">{{ completionStats.monthCompleted }}</span>
      </div>
      <div class="setting-stats-item">
        <span class="setting-stats-label">平均完成时间:</span>
        <span class="setting-stats-value">{{ formatDuration(completionStats.averageCompletionTime) }}</span>
      </div>
    </div>

    <!-- 效率统计 -->
    <div v-if="efficiencyStats" class="setting-stats-container">
      <h3 class="setting-stats-title">效率统计</h3>
      <div class="setting-stats-item">
        <span class="setting-stats-label">平均任务执行时间:</span>
        <span class="setting-stats-value">{{ formatDuration(efficiencyStats.averageExecutionTime) }}</span>
      </div>
      <div class="setting-stats-item">
        <span class="setting-stats-label">最快完成时间:</span>
        <span class="setting-stats-value">{{ formatDuration(efficiencyStats.fastestCompletion) }}</span>
      </div>
      <div class="setting-stats-item">
        <span class="setting-stats-label">最慢完成时间:</span>
        <span class="setting-stats-value">{{ formatDuration(efficiencyStats.slowestCompletion) }}</span>
      </div>
      <div class="setting-stats-item">
        <span class="setting-stats-label">完成率:</span>
        <span class="setting-stats-value">{{ (efficiencyStats.completionRate * 100).toFixed(1) }}%</span>
      </div>
    </div>

    <!-- 日志统计 -->
    <div v-if="logStats" class="setting-stats-container">
      <h3 class="setting-stats-title">日志统计</h3>
      <div class="setting-stats-item">
        <span class="setting-stats-label">总日志记录数:</span>
        <span class="setting-stats-value">{{ logStats.totalLogs }}</span>
      </div>
      <div class="setting-stats-item">
        <span class="setting-stats-label">最近变化数:</span>
        <span class="setting-stats-value">{{ logStats.recentChangesCount }}</span>
      </div>
      <div v-if="logStats.lastChange" class="setting-stats-item">
        <span class="setting-stats-label">最后变化时间:</span>
        <span class="setting-stats-value">{{ formatDateTime(logStats.lastChange) }}</span>
      </div>
    </div>
